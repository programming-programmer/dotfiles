...
*** DONE add registers for common files
*** TODO add hook to delete Dired buffer when jumping to file
*** DONE Arrange Headings to make more sense
 * Especially org-mode and related things
...
** Load Lisp Files
#+begin_src emacs-lisp
  (load (concat user-emacs-directory "lisp/funcs.el"))
  (load (concat user-emacs-directory "lisp/var.el"))
#+end_src
...
#+begin_src emacs-lisp
  ...
  "C-r" '(jump-to-register :which-key "jump to register")
#+end_src
...
*** Registers
#+begin_src emacs-lisp
  "j" '(nil :which-key "register")
  
  "ji" '(point-to-register :which-key "insert register")
#+end_src
